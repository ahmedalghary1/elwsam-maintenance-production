...
chcp 65001 >NUL
title Production Supervisor - Release APK Builder
...
set "KEYSTORE=%~dp0.signing\production-release.jks"
set "UNSIGNED_APK=%~dp0app\build\outputs\apk\release\app-release.apk"
set "TEMP_DIR=%~dp0.build-temp"
set "ALIGNED_APK=%~dp0.build-temp\production-release-aligned.apk"
set "SIGNED_TEMP=%~dp0.build-temp\production-supervisor-release.apk"
set "FINAL_APK=%~dp0release-output\production-supervisor-release.apk"

echo ============================================================
echo Building signed Production Supervisor APK
echo ============================================================
...
if not exist "%KEYSTORE%" (
    ...
    echo %KEYSTORE%
    echo.
    "%KEYTOOL%" -genkeypair -v -keystore "%KEYSTORE%" -alias production -keyalg RSA -keysize 4096 -validity 10000 -dname "CN=Production, O=Elwsam, C=EG" -storepass elwsam123 -keypass elwsam123
...
call "%APKSIGNER%" sign --ks "%KEYSTORE%" --ks-pass pass:elwsam123 --ks-key-alias production --out "%SIGNED_TEMP%" "%ALIGNED_APK%"
if errorlevel 1 goto failed
...
:missing_java
echo ERROR: JDK was not found at:
echo %JAVA_HOME%
goto failed
